...
class ForgotPasswordController {
  async store(req, res) {
    ...
    return res.status(200).send({ message: 'Password reset link has been successfully sent to your inbox' });
      ...
      res.status(400).send({ error: 'Erro em sua tentativa de recuperação de senha, tente novamente'})
  ...
  async update(req, res, next) {
    try {
      ...
      return res.status(200).send({ token, user: { id, name, email, hash, password } });
    } catch (e) {
